...
export default function CourseCard({ course, index, onMutateCourse }) {
  ...
  function toggleTask(id) {
    onMutateCourse(index, tasks => tasks.map(task => task.id === id ? { ...task, isDone: !task.isDone } : task));
  }
  ...
  return (
    <article className="course card">
      <header className="cardHeader">
        <h2>{course.title}</h2>
        {(course.tasks.length > 0 && course.tasks.every((task) => task.isDone)) && <span className="badge">All caught up</span>}
      </header>
